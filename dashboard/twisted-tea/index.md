...
<title>The Twisted Tea Incident Registry</title>
<meta name="description" content="A satirical, fully-sourced tracker of documented assaults carried out with a can of Twisted Tea, since December 2020.">
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  @media (prefers-color-scheme: dark){
    body{ background:#1B140B; color:#EFE6D2; }
  }
  ...
  @media (prefers-color-scheme: dark){ .stat{background:#1B140B;} }
  .stat .n{
    font-family:"Lora",Georgia,serif;
    font-weight:700;
    font-size:2.1rem;
  ...
  .barchart{margin-top:8px;}
  .bar-row{display:flex;align-items:center;gap:14px;margin-bottom:12px;}
  .bar-label{width:120px;flex:none;font-family:"IBM Plex Mono",monospace;font-size:0.85rem;}
  .bar-track{flex:1;background:var(--paper-dark);height:22px;position:relative;border:1px solid var(--line);}
  @media (prefers-color-scheme: dark){ .bar-track{background:#2A2013;} }
  .bar-fill{height:100%;background:var(--amber);}
  .bar-fill.provisional{background:repeating-linear-gradient(45deg,var(--amber),var(--amber) 6px,var(--paper-dark) 6px,var(--paper-dark) 12px);}
  .bar-count{width:34px;flex:none;font-family:"IBM Plex Mono",monospace;font-size:0.85rem;text-align:right;}
  .chart-note{font-size:0.85rem;color:var(--ink-soft);margin-top:10px;}
  .fig{margin:0 0 34px;}
  .fig:last-child{margin-bottom:0;}
  .fig-caption{font-size:0.85rem;color:var(--ink-soft);margin-top:10px;max-width:64ch;}

  .map-wrap{max-width:680px;margin:0 auto;}
  .us-map{width:100%;height:auto;display:block;}
  .map-loading{font-family:"IBM Plex Mono",monospace;font-size:14px;fill:var(--ink-soft);}
  .state{stroke:var(--ink);stroke-width:0.6;fill:var(--paper-dark);cursor:pointer;transition:opacity 0.1s;}
  @media (prefers-color-scheme: dark){ .state{fill:#2A2013;} }
  .state:hover{opacity:0.75;stroke-width:1.4;}
  .state-confirmed{fill:var(--amber);}
  .state-provisional{fill:url(#hatch);}
  .state-excluded{fill:var(--paper);stroke-dasharray:2 2;}
  @media (prefers-color-scheme: dark){ .state-excluded{fill:#1B140B;} }
  .map-legend{
    display:flex;
    flex-wrap:wrap;
    gap:16px;
  ...
  .map-legend .dot{width:11px;height:11px;border-radius:50%;display:inline-block;margin-right:6px;vertical-align:middle;}
  .dot-confirmed{background:var(--amber);}
  .dot-provisional{background:repeating-linear-gradient(45deg,var(--amber),var(--amber) 2px,var(--paper-dark) 2px,var(--paper-dark) 4px);}
  .dot-context{background:var(--paper);border:1px dashed var(--ink-soft);}
  .dot-none{background:var(--paper-dark);border:1px solid var(--line);}
  ...
  [data-info]:focus{outline:2px solid var(--amber-deep);outline-offset:3px;}
  .state:focus{outline:2px solid var(--amber-deep);}
  ...
  .vbar-col{display:flex;flex-direction:column;align-items:center;justify-content:flex-end;flex:1;height:100%;}
  .vbar-val{font-family:"IBM Plex Mono",monospace;font-size:0.8rem;margin-bottom:6px;}
  .vbar{width:100%;max-width:42px;background:var(--amber);}
  .vbar.provisional{background:repeating-linear-gradient(45deg,var(--amber),var(--amber) 6px,var(--paper-dark) 6px,var(--paper-dark) 12px);}
  .vbar.zero{background:var(--paper-dark);border:1px solid var(--line);}
  .vbar-year{font-family:"IBM Plex Mono",monospace;font-size:0.78rem;margin-top:8px;color:var(--ink-soft);}
  ...
  @media (prefers-color-scheme: dark){ th{background:#2A2013;} }
  .case-block{margin-bottom:26px;border:1px solid var(--ink);}
  .case-block:last-child{margin-bottom:0;}
  .case-block table{border:none;}
  .case-block th,.case-block td{border:none;border-bottom:1px solid var(--line);}
  .case-id{
    font-family:"IBM Plex Mono",monospace;
    background:var(--ink);
    color:var(--paper);
  ...
  .status-pill.provisional{background:var(--paper-dark);}

  .excluded-table th{width:110px;}
  .why{color:var(--brick);}
  ...
  .methodology dd{margin:6px 0 0;max-width:68ch;}
  ...
  @media (prefers-color-scheme: dark){ .origin{background:#2A2013;} }
  .origin p{margin:0 0 10px;}
  .origin p:last-child{margin-bottom:0;}

  .downloads{display:flex;gap:12px;flex-wrap:wrap;margin-top:6px;}
  .dl-btn{
    font-family:"IBM Plex Mono",monospace;
    font-size:0.85rem;
    border:1px solid var(--ink);
  ...
  @media (prefers-color-scheme: dark){ .dl-btn{background:#1B140B;color:#EFE6D2;border-color:#EFE6D2;} }
  ...
  .sources-list{margin:6px 0 0;padding-left:18px;font-size:0.85rem;}
...
<div class="wrap">
  ...
  <header class="hero">
    <p class="tag">Registry &middot; Satire, mostly sourced</p>
    <h1>The Twisted Tea Incident Registry</h1>
    <p class="dek">Every documented case in the United States in which a can of Twisted Tea was used as an
      improvised weapon, tracked since the format was invented at a Circle K in Elyria, Ohio, on Christmas Eve 2020.</p>
    <div class="meta-row">
      <span>Last updated 2026-09-18</span>
      <span>n = 2 confirmed, 1 provisional</span>
      <span>Coverage: national</span>
      <span>Peer reviewed: no</span>
    </div>
  </header>

  <section id="stats">
    <div class="stats">
      <div class="stat"><span class="n">2</span><span class="l">Confirmed incidents</span></div>
      <div class="stat"><span class="n">1</span><span class="l">Provisional (weapon disputed)</span></div>
      <div class="stat"><span class="n">3</span><span class="l">States reporting, of 50</span></div>
      <div class="stat"><span class="n">5M+</span><span class="l">Views on the origin video</span></div>
    </div>
  </section>
  ...
  <section id="chart">
    ...
    <figure class="fig">
      <div class="map-wrap">
        <svg id="us-map" viewBox="-60 0 1030 620" class="us-map" role="img" aria-label="Map of the United States, colored by whether a Twisted Tea incident has been documented there">
          <defs>
            <pattern id="hatch" width="8" height="8" patternTransform="rotate(45)" patternUnits="userSpaceOnUse">
              <rect width="8" height="8" fill="var(--amber)"></rect>
              <line x1="0" y1="0" x2="0" y2="8" stroke="var(--paper-dark)" stroke-width="4"></line>
            </pattern>
          </defs>
        </svg>
        <div class="map-legend">
          <span><i class="dot dot-confirmed"></i> Confirmed incident</span>
          <span><i class="dot dot-provisional"></i> Provisional incident</span>
          <span><i class="dot dot-context"></i> Excluded case (context only)</span>
          <span><i class="dot dot-none"></i> No documented incident</span>
        </div>
      </div>
      <figcaption class="fig-caption"><em>Figure 1. Hover or tap a state for its case detail. Forty-five states
        report nothing at all, which given the sample size is not much of a finding.</em></figcaption>
    </figure>

    <figure class="fig">
      <div class="vbarchart">
        <div class="vbar-col" tabindex="0" data-info="<strong>2020</strong><br>2 confirmed incidents<br>TTA-2020-001 &middot; Elyria, OH<br>TTA-2020-002 &middot; Merrimack, NH"><span class="vbar-val">2</span><div class="vbar" style="height:130px"></div><span class="vbar-year">2020</span></div>
        <div class="vbar-col" tabindex="0" data-info="<strong>2021</strong><br>1 provisional incident<br>TTA-2021-003 &middot; Port Charlotte, FL (weapon disputed)"><span class="vbar-val">1</span><div class="vbar provisional" style="height:65px"></div><span class="vbar-year">2021</span></div>
        <div class="vbar-col" tabindex="0" data-info="<strong>2022</strong><br>No incidents recorded"><span class="vbar-val">0</span><div class="vbar zero" style="height:4px"></div><span class="vbar-year">2022</span></div>
        <div class="vbar-col" tabindex="0" data-info="<strong>2023</strong><br>No incidents recorded"><span class="vbar-val">0</span><div class="vbar zero" style="height:4px"></div><span class="vbar-year">2023</span></div>
        <div class="vbar-col" tabindex="0" data-info="<strong>2024</strong><br>No incidents recorded"><span class="vbar-val">0</span><div class="vbar zero" style="height:4px"></div><span class="vbar-year">2024</span></div>
        <div class="vbar-col" tabindex="0" data-info="<strong>2025</strong><br>No incidents recorded"><span class="vbar-val">0</span><div class="vbar zero" style="height:4px"></div><span class="vbar-year">2025</span></div>
        <div class="vbar-col" tabindex="0" data-info="<strong>2026</strong><br>No incidents recorded (yet)"><span class="vbar-val">0</span><div class="vbar zero" style="height:4px"></div><span class="vbar-year">2026</span></div>
      </div>
      <figcaption class="fig-caption"><em>Figure 2. Incidents by year &mdash; hover or tap a column. The registry
        opens with two cases 13 months apart in December 2020 and January 2021, then five years of nothing.
        Whether that is a resolved problem or an uncovered one is not a question a registry with n = 3 can
        answer.</em></figcaption>
    </figure>
    <figure class="fig">
      <div class="barchart">
        <div class="bar-row" tabindex="0" data-info="<strong>Charged</strong><br>TTA-2020-002 &middot; Merrimack, NH (simple assault, domestic violence)<br>TTA-2021-003 &middot; Port Charlotte, FL (aggravated assault, provisional)">
          <span class="bar-label">Charged</span>
          <div class="bar-track"><div class="bar-fill" style="width:100%"></div></div>
          <span class="bar-count">2</span>
        </div>
        <div class="bar-row" tabindex="0" data-info="<strong>No charges filed</strong><br>TTA-2020-001 &middot; Elyria, OH &mdash; no charges, but 5M+ YouTube views instead">
          <span class="bar-label">No charges</span>
          <div class="bar-track"><div class="bar-fill" style="width:50%"></div></div>
          <span class="bar-count">1</span>
        </div>
      </div>
      <figcaption class="fig-caption"><em>Figure 3. Charges filed, where reported &mdash; hover or tap a bar. The
        one case that went uncharged is also the one with 5 million YouTube views, which is its own small
        commentary on what gets prosecuted versus what gets memed.</em></figcaption>
    </figure>
  </section>
  ...
  <section id="incidents">
    ...
    <div class="case-block">
      <table>
        <tr><th>Case ID</th><td><span class="case-id">TTA-2020-001</span></td></tr>
        <tr><th>Date</th><td>December 24, 2020</td></tr>
        <tr><th>Location</th><td>Circle K, Elyria, Ohio</td></tr>
        <tr><th>Weapon</th><td>24 oz can, swung one-handed at the head</td></tr>
        <tr><th>Context</th><td>An unmasked man at the checkout counter directed racial slurs and taunts at
          another customer, later identified as Barry Allen, and goaded him to fight.</td></tr>
        <tr><th>Sequence</th><td>Allen, who appeared to want only to pay for his tallboy and leave, struck the man
          with the can and continued the altercation on the ground.</td></tr>
        <tr><th>Charges</th><td>None filed, per local reporting</td></tr>
        <tr><th>Outcome</th><td>Video passed 5 million views on YouTube alone within weeks and became the
          namesake meme ("Tea-KO"). This is the registry's index case.</td></tr>
        <tr><th>Sources</th><td>
          <a href="https://en.wikipedia.org/wiki/Twisted_Tea" target="_blank" rel="noopener">Wikipedia</a> &middot;
          <a href="https://fingers.email/p/the-tea-ko-memed-round-the-world" target="_blank" rel="noopener">Fingers</a> &middot;
          <a href="https://vinepair.com/articles/ntk-twisted-tea-guide/" target="_blank" rel="noopener">VinePair</a>
        </td></tr>
      </table>
    </div>
    ...
    <div class="case-block">
      <table>
        <tr><th>Case ID</th><td><span class="case-id">TTA-2020-002</span></td></tr>
        <tr><th>Date</th><td>January 18, 2020</td></tr>
        <tr><th>Location</th><td>Merrimack, New Hampshire</td></tr>
        <tr><th>Weapon</th><td>24 oz can, struck to the face</td></tr>
        <tr><th>Context</th><td>A domestic dispute; the victim reported the incident to an officer stationed
          nearby.</td></tr>
        <tr><th>Charges</th><td>Simple assault and domestic violence</td></tr>
        <tr><th>Outcome</th><td>Facial bruising reported. Suspect surrendered after a warrant was issued.</td></tr>
        <tr><th>Sources</th><td>
          <a href="https://whdh.com/news/police-nh-woman-facing-assault-charge-after-hitting-person-in-face-with-large-can-of-twisted-tea/" target="_blank" rel="noopener">WHDH</a>
        </td></tr>
      </table>
    </div>
    ...
    <div class="case-block">
      <table>
        <tr><th>Case ID</th><td><span class="case-id">TTA-2021-003</span> <span class="status-pill provisional">provisional</span></td></tr>
        <tr><th>Date</th><td>January 19, 2021</td></tr>
        <tr><th>Location</th><td>Peachland Promenade, Port Charlotte, Charlotte County, Florida</td></tr>
        <tr><th>Weapon</th><td>Can, carried in hand; a 28-inch steel pipe was also carried</td></tr>
        <tr><th>Context</th><td>The suspect, Shane Gillis, made threatening phone calls to a former employer before
          arriving at the scene, telling him <q>I&rsquo;m coming for you straight through the parking lot.</q></td></tr>
        <tr><th>Charges</th><td>Aggravated assault with a deadly weapon, without intent to kill</td></tr>
        <tr><th>Why provisional</th><td class="why">Reporting does not specify which item the charge treats as
          the deadly weapon. The pipe is the likelier candidate; the can rides along on reputation. The
          Charlotte County Sheriff's Office itself joked on Facebook that it couldn't say whether the tea
          prompted the confrontation or was simply the weapon of choice.</td></tr>
        <tr><th>Sources</th><td>
          <a href="https://www.gulfcoastnewsnow.com/article/florida-man-threatens-to-kill-former-employer-with-twisted-tea-metal-pipe/46729993" target="_blank" rel="noopener">Gulf Coast News Now</a> &middot;
          <a href="https://www.aol.com/news/florida-man-arrested-threatening-ex-003844145.html" target="_blank" rel="noopener">AOL/WINK News</a>
        </td></tr>
      </table>
    </div>
  </section>
  ...
  <section id="excluded">
    ...
    <p>Searching "Twisted Tea" and "assault" mostly returns theft reports, unrelated hot-tea incidents, and a
      genuinely large number of convenience-store robberies. Each near miss is logged with the reason it failed,
      because a registry that only shows what it counted hides the interesting part.</p>
    <table class="excluded-table">
      <thead><tr><th>Date</th><th>Location</th><th>What happened</th><th>Why excluded</th><th>Source</th></tr></thead>
      <tbody>
        <tr>
          <td>Jul 22, 2023</td>
          <td>Bakersfield, CA</td>
          <td>Two suspects stole Twisted Tea from a market and punched the manager when confronted; both
            convicted of robbery.</td>
          <td>The can was the stolen property, not the weapon. The manager was struck with a fist.</td>
          <td><a href="https://www.kget.com/news/crime-watch/altercation-over-stolen-twisted-tea-results-in-felony-convictions/amp/" target="_blank" rel="noopener">KGET</a></td>
        </tr>
        <tr>
          <td>Undated</td>
          <td>Logan, Philadelphia, PA</td>
          <td>A man threw a cup of hot tea in a restaurant employee's face, causing second- and third-degree
            burns.</td>
          <td>Brand not identified as Twisted Tea, and the liquid was thrown rather than the can swung.</td>
          <td><a href="https://6abc.com/4681082/" target="_blank" rel="noopener">6abc</a></td>
        </tr>
        <tr>
          <td>Undated</td>
          <td>Feasterville, PA</td>
          <td>Two women loaded a shopping cart with Twisted Tea and fled an Acme without paying.</td>
          <td>No altercation occurred. Theft only; registry tracks assaults.</td>
          <td><a href="https://www.cbsnews.com/philadelphia/news/tea-time-police-searching-for-2-women-accused-of-stealing-shopping-cart-full-of-twisted-tea/" target="_blank" rel="noopener">CBS Philly</a></td>
        </tr>
      </tbody>
    </table>
  </section>
  ...
  <section id="origin">
    <h2>The index case</h2>
    <div class="origin">
      <p>Every entry in this registry exists downstream of one video. On December 24, 2020, footage from a
        Circle K in Elyria, Ohio, showed a man hurling racial slurs at another customer until the customer,
        Barry Allen, hit him with a tallboy of Twisted Tea. No charges were filed. The video became one of the
        most-shared clips of that December, and "using a Twisted Tea can as a weapon" became internet shorthand
        for justified retaliation long before anyone had reason to check whether it kept happening.</p>
      <p>It has happened at least twice more since, for reasons that have nothing to do with racism or memes:
        a domestic dispute and a workplace confrontation. Whether that is a real pattern or just three unrelated
        news stories that happen to share a beverage brand is exactly the kind of question a registry with n = 3
        cannot answer, which is the point.</p>
    </div>
  </section>
  ...
  <section id="methodology">
    ...
    <dl class="methodology">
      ...
      <dd>A can of Twisted Tea, opened or unopened, used as a striking implement against a person during an
        altercation. The can must do the hitting. Being present, being stolen, or merely being consumed
        beforehand does not qualify.</dd>
      <dt>Data sources</dt>
      <dd>Local news reporting and, where available, court records. No agency tracks beverage-based improvised
        weapons, so this is assembled by hand from television and newspaper archives, the same way the donkey
        one was.</dd>
      <dt>Search protocol</dt>
      <dd>Queries combining "Twisted Tea" with "assault," "hit," "struck," "weapon," and "charged," across all
        available years. The protocol returned a great many theft reports, one unrelated hot-tea burn case, and
        the original viral video roughly as often as it returned anything new.</dd>
      <dt>Coding</dt>
      <dd>Cases are marked confirmed when a named location, a specific date, and a described strike are all
        present in reporting. Cases missing one of those are marked provisional.</dd>
      <dt>Limitations</dt>
      <dd>The sample is small enough that any trend line is decorative. A 2020 cluster reflects two unrelated
        incidents six years ago, not a wave. Absence of a state from this list means no reported case was found,
        not that none occurred.</dd>
    </dl>
  </section>

  <section id="download">
    <h2>Download the data</h2>
    <p>Confirmed and provisional cases, plus the excluded log, in open formats.</p>
    <div class="downloads">
      <a class="dl-btn" href="twisted-tea-incidents.csv" download>Download CSV</a>
      <a class="dl-btn" href="twisted-tea-incidents.json" download>Download JSON</a>
    </div>
  </section>

  <footer>
    <p>Data current as of 2026-09-18. Confirmed cases: 2. Provisional: 1. Cases that almost counted: 3.</p>
    <p>Have a case that belongs here? It needs a named location, a date, and a news source describing the can
      actually being used to strike someone.</p>
  </footer>
...
(function(){
  ...
  // Bar charts (years + charges): plain HTML elements, works via mouse and keyboard focus.
  document.querySelectorAll('[data-info]').forEach(function(el){
    el.addEventListener('mousemove', function(e){ showTooltip(el.dataset.info, e.pageX, e.pageY); });
    el.addEventListener('mouseleave', hideTooltip);
    el.addEventListener('focus', function(){
      var r = el.getBoundingClientRect();
      showTooltip(el.dataset.info, r.left + window.scrollX, r.bottom + window.scrollY);
    });
    el.addEventListener('blur', hideTooltip);
    el.addEventListener('touchstart', function(e){
      var t = e.touches[0];
      showTooltip(el.dataset.info, t.pageX, t.pageY);
    });
  });

  // US map: state boundaries are embedded inline (no runtime fetch needed, so this
  // works even opened as a local file), pre-projected with the Albers USA projection
  // used by the public us-atlas dataset. d3.geoPath() needs no projection call because
  // the coordinates are already in that projected pixel space.
  var caseData = {
    'Ohio': { status: 'confirmed', html: '<strong>Ohio</strong><br>1 confirmed incident<br>TTA-2020-001 &middot; Circle K, Elyria &middot; Dec 24, 2020' },
    'New Hampshire': { status: 'confirmed', html: '<strong>New Hampshire</strong><br>1 confirmed incident<br>TTA-2020-002 &middot; Merrimack &middot; Jan 18, 2020' },
    'Florida': { status: 'provisional', html: '<strong>Florida</strong><br>1 provisional incident<br>TTA-2021-003 &middot; Port Charlotte &middot; Jan 19, 2021 (weapon disputed)' },
    'California': { status: 'excluded', html: '<strong>California</strong><br>1 excluded case<br>Bakersfield, 2023 &mdash; theft, weapon was a fist' },
    'Pennsylvania': { status: 'excluded', html: '<strong>Pennsylvania</strong><br>2 excluded cases<br>Philadelphia hot-tea assault (brand mismatch)<br>Feasterville theft (no assault)' }
  };

  try {
    var us = JSON.parse(document.getElementById('us-topo-data').textContent);
    ...
    svg.selectAll('path.state')
      ...
      .attr('class', function(d){
        var info = caseData[d.properties.name];
        return 'state' + (info ? ' state-' + info.status : '');
      })
      .attr('tabindex', 0)
      .attr('d', path)
      .on('mousemove', function(event, d){
        var info = caseData[d.properties.name];
        var html = info ? info.html : '<strong>' + d.properties.name + '</strong><br>No documented incidents';
      ...
      .on('focus', function(event, d){
        ...
        showTooltip(html, r.left + window.scrollX, r.top + window.scrollY);
      })
      .on('blur', hideTooltip);
  } catch (e){
    document.querySelector('.map-wrap').insertAdjacentHTML('beforeend',
      '<p class="chart-note">Map failed to render (' + e.message + '). It needs d3 and topojson-client to load from the CDN — check your connection or an ad blocker.</p>');
  }
})();
</script>
</body>
